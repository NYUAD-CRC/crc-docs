...
.. code-block:: bash
    ...
    ./process-one-result file.1	
    ./process-one-result file.2	
    ./process-one-result file.3	
    ...	
...
.. admonition:: More on the **TimeLimit** parameter
    ...
    Now each group of job will run on one node and the parallel job array is submitted by default to the compute nodes which has 128 CPUs per node.
    ...
    **Hence, while submitting the ``parallel job array`` it will be good to specify this TimeLimit as well, as
    ...
    ::
        ...
        >> slurm_parallel_ja_submit.sh -N 8 -t 00:50:00
...
.. code-block:: bash
    ...
    $> slurm_parallel_ja_submit.sh -N 8 -t 00:50:00 list_of_commands.txt
